...
class MainTabBarController: UITabBarController {
    ...
    convenience init(friendsCache: FriendsCache) {
        ...
        self.friendsCache = friendsCache // This should come before setupViewController is called.
...
//            makeNav(for: makeCardsList(), title: "Cards", icon: "creditcard.fill")
    ...
    private func makeFriendsList() -> ListViewController {
        ...
        let cacheservice: ItemsService = FriendsCacheItemsServiceAdapter(cache: friendsCache) { [weak vc] item in
            vc?.select(friend: item)
        }
        let apiservice: ItemsService = FriendsAPIItemsServiceAdapter(
        ...
        vc.service = isPremium ? apiservice.fallback(cacheservice) : apiservice
        return vc
    }
    ...
    private func makeSentTransfersList() -> ListViewController {
        ...
        let service = SentTransfersAPIItemsServiceAdapter(api: TransfersAPI.shared, select: { [weak vc] item in
    ...
    private func makeReceivedTransfersList() -> ListViewController {
        ...
        let service = ReceivedTransfersAPIItemsServiceAdapter(api: TransfersAPI.shared, select: { [weak vc] item in
            vc?.select(transfer: item)
        })
        vc.service = service.retry(1)
        return vc
    ...
    private func makeCardsList() -> ListViewController {
        ...
        vc.service = CardAPIItemsServiceAdapter(api: CardAPI.shared, select: { [weak vc] item in
            vc?.select(card: item)
        })
        return vc
    }
...
extension ItemsService {
    ...
    // unsigned int because we don't want negative numbers
